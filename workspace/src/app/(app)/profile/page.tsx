...
// New component to handle Stripe redirect logic with Suspense
function StripeStatusHandler() {
  ...
  useEffect(() => {
    ...
    if (stripeStatus === 'success' && stripeSessionId) {
      ...
      // Clean the URL by removing Stripe query parameters
...
export default function ProfilePage() {
  ...
  const { toast } = useToast();
  // useRouter and useSearchParams are now in StripeStatusHandler
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <Suspense fallback={<></>}> {/* Suspense wrapper for the handler */}
        <StripeStatusHandler />
      </Suspense>
