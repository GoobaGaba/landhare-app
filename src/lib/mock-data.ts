...
export const updateBookingStatus = async (bookingId: string, status: Booking['status']): Promise<Booking | undefined> => {
  ...
  if (paymentTxnIndex !== -1) {
      if (status === 'Confirmed') currentDb.transactions[paymentTxnIndex].status = 'Completed';
      if (status === 'Declined' || status === 'Cancelled by Renter') currentDb.transactions[paymentTxnIndex].status = 'Failed';
  }
  ...
  if (status === 'Confirmed') {
    ...
    const paymentTxn = currentDb.transactions.find(t => t.relatedBookingId === booking.id && t.type === 'Booking Payment');
    if (paymentTxn) {
...
// Functions below are not used by the live app but might be useful for other contexts
// They are kept for potential future use or debugging.
