...
export const updateUserProfile = async (userId: string, data: Partial<User>): Promise<User | undefined> => {
    if (firebaseInitializationError) {
        ...
        if (userIndex !== -1) {
            ...
            // Handle subscription change logic for mock mode
            if (data.subscriptionStatus && currentUserData.subscriptionStatus !== data.subscriptionStatus) {
                ...
                if (newStatus === 'premium') {
                    ...
                    currentUserData.walletBalance -= PREMIUM_SUBSCRIPTION_PRICE;
                    db.metrics.totalSubscriptionRevenue += PREMIUM_SUBSCRIPTION_PRICE;
                } else { // Downgrading
                    transactionAmount = PREMIUM_SUBSCRIPTION_PRICE;
            ...
            db.users[userIndex] = { ...currentUserData, ...data };
            saveMockDb(db);
